...
contract('Test MyNFT contract', function(accounts) {
    ...
    it('delpoy and mint ERC721 token', async() => {
    ...
    it('safe transfer', async() => {
        ...
        const wrongOwner = token.safeTransferFrom(account1, account3, tokenId2, { from: accounts[1] }) // tokenId
        expect(wrongOwner).to.be.rejectedWith(/VM Exception while processing transaction: revert/)

        const wrongFromGas = token.safeTransferFrom(account2, account3, tokenId2, { from: accounts[1] }) // tokenId
